...
async function testEmailNotifications() {
  try {
    ...
    const eligibleUsers = users.filter(user => user.notifications?.newEvents !== false);
    console.log(`📊 ${eligibleUsers.length} out of ${users.length} users are eligible for notifications\n`);
    ...
    const testEvent = {
      ...
      description: 'This is a test event to verify that users receive email notifications when events are approved.',
      category: 'Technology',
    ...
    console.log(`   Recipients: ${eligibleUsers.map(u => u.email).join(', ')}`);
    console.log('');
    
    // Send the notification
    await sendEventApprovalNotification(eligibleUsers, testEvent, testOrganizer);
    ...
    console.log('   4. Ensure your email provider allows emails from team.showpass@gmail.com');
